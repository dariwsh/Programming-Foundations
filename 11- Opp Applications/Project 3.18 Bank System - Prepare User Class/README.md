# 👥 Bank System — Lesson 18: User Domain Model (`clsUser`)

> **Milestone:** Project 3: Bank System — Lesson #18 (Prepare User Class)  
...
Engineered the `clsUser` class extending `clsPerson` with user credentials, bitwise permission masks (`enPermissions`), and flat-file persistence in `Users.txt`.

---

## 🎯 Key Concepts & Implementation Highlights

- User Domain Modeling inheriting from `clsPerson`
- Bitwise Permission Flags (`eAll = -1`, `pListClients = 1`, `pAddNewClient = 2`, etc.)
- `CheckAccessPermission(enPermissions)` helper method
- CRUD operations and `Users.txt` serialization

---

## 🗂️ Key Files & Architecture

- **Primary Classes / Headers:** `clsUser.h`, `clsPerson.h`, `clsString.h`
