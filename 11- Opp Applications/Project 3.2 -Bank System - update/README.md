# 🏦 Bank System — Lesson 02: Update Client

> **Milestone:** Project 3: Bank System — Lesson #02  
...
Implements the client update lifecycle in memory and persistent storage using the Mode pattern (`UpdateMode`) and `Save()` pipeline.

---

## 🎯 Key Concepts & Implementation Highlights

- Internal Object Mode Management (`enMode { EmptyMode, UpdateMode, AddNewMode }`)
- Encapsulated `Save()` method returning operation status (`enSaveResults`)
- File updating via record streaming and selective replacement
- Input property setters with validation

---

## 🗂️ Key Files & Architecture

- **Primary Classes / Headers:** `clsBankClient.h`, `clsPerson.h`, `clsString.h`
